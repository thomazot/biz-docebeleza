...
$j(document)
    .ready(function ($) {
        ...
        var toolbar = $('.col-main > .toolbar .sorter')

        if (toolbar.length) {
            var ButtonFilter = $(
                '<button class="filters__open">Filtrar</button>'
            )
            ButtonFilter.click(function () {
                $('.filters > .title').trigger('click')
            })
            toolbar.append(ButtonFilter)
        }
    })
    .on('resizeStop', function (e) {
        // Safe window.resize
...
function mycart($) {
    var head = $('<div class="mycart__head"></div>')
    var title = $('<div class="mycart__title">Seu Carrinho</div>')
    var info = $(
        '<div class="mycart__info"><span class="mycart__label-qtd"><span class="qtd">0</span> Produtos |</span> <span class="price">R$ 0,00</span><a href="/checkout/cart"><svg class="ico z-next"><use xlink:href="#z-next" /></svg></a></div>'
    )
    var mycartHeader = $('.mycart__header a')
    var mycartContent = $('.mycart__content')

    title.prepend('<svg class="ico z-prev"><use xlink:href="#z-prev" /></svg>')
    head.prepend(title).append(info)

    if (mycartContent) mycartContent.prepend(head)
    if (mycartHeader) {
        var qtd = mycartHeader.find('.qtd').text()
        var price = mycartContent.find('.mycart__totals .regular-price .price')

        if (Number(qtd) === 0) $('.mycart').addClass('mycart--empty')
        else $('.mycart').addClass('mycart--not-empty')

        info.find('.qtd').text(qtd)
        if (price.length) {
            info.find('.price').text(price.text())
        }

        $('.mycart__actions').prepend(
            '<button class="mycart__close" type="">Voltar</button>'
        )

        mycartHeader.click(function (event) {
            event.preventDefault()
            $('body').toggleClass('mycart--open')
        })
        $('.mycart__title').click(function (event) {
            event.preventDefault()
            $('body').toggleClass('mycart--open')
        })

        $('.mycart').click(function (event) {
            if ($(event.target).hasClass('mycart')) {
                $('body').toggleClass('mycart--open')
            }
        })

        $('.mycart__close').click(function (event) {
            event.preventDefault()
            $('body').toggleClass('mycart--open')
        })
    }
